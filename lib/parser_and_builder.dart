...
typedef ParserAndBuilder<Parsed, Raw>
    = ParserAndBuilderInner<Parsed, Inv<Parsed>, Raw, Inv<Raw>>;

@immutable
class ParserAndBuilderInner<Parsed, ParsedInv extends Inv<Parsed>, Raw,
    RawInv extends Inv<Raw>> {
  const ParserAndBuilderInner.custom({
    ...
    ParserAndBuilder<(PathSegments, Query), Uri>.custom(
      ...
      builder: (value) {
        return uriAbsolute(
          ...
          queryParameters: queryParserAndBuilder.builder(value.$2),
        );
      },
    );
...
/// uriAbsolute(pathSegments: ['path'], queryParameters: IMap({'position': IList(['left']), 'empty': IList(['']), '': IList(['emptyValue'])})) // '/path?position=left&empty&=emptyValue'
/// ```
///
